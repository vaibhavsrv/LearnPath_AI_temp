    ...
    <div className="page-wrapper">
      ...
      <main className="container" style={{ paddingTop: 64, paddingBottom: 40 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          {domains.map(d => (
            <button key={d} className={`btn btn-sm`} style={{ background: filter === d ? DOMAIN_COLORS[d] : 'var(--color-card)', color: filter === d ? '#fff' : 'var(--color-ink)', border: `1px solid ${DOMAIN_COLORS[d]}` }} onClick={() => setFilter(d)}>
              {DOMAIN_NAMES[d] || d}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 16 }}>
          <div style={{ flex: 1, background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 12, overflow: 'hidden', position: 'relative', height: 520, cursor: dragging ? 'grabbing' : 'grab' }} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onWheel={handleWheel}>
            ...
            <svg ref={svgRef} width="100%" height="100%" viewBox={`0 0 1200 800`} style={{ transform: `scale(${zoom}) translate(${pan.x / zoom}px, ${pan.y / zoom}px)` }}>
              <defs>
                <marker id="arrow" viewBox="0 0 10 6" refX="10" refY="3" markerWidth="8" markerHeight="6" orient="auto"><path d="M0,0 L10,3 L0,6" fill="var(--color-ink-3)" fillOpacity="0.3" /></marker>
              </defs>
              {skills.map(s => {
                ...
                return s.prerequisites.map(pId => {
                  ...
                  return <line key={`${s.id}-${pId}`} x1={to.x} y1={to.y} x2={from.x} y2={from.y} stroke="var(--color-ink-3)" strokeOpacity={visible ? 0.3 : 0.08} strokeWidth={1} markerEnd="url(#arrow)" />;
                });
              })}
              {skills.map(s => {
                ...
                return (
                  <g key={s.id} onClick={() => setSelected(isSelected ? null : s.id)} onMouseEnter={() => setHovered(s.id)} onMouseLeave={() => setHovered(null)} style={{ cursor: 'pointer', opacity: visible ? 1 : 0.15 }}>
                    ...
                    {visible && <text x={pos.x} y={pos.y + 28} textAnchor="middle" fontSize={9} fill="var(--color-ink)" fontWeight={isSelected ? 700 : 400} style={{ pointerEvents: 'none' }}>{s.name.length > 18 ? s.name.substring(0, 16) + '…' : s.name}</text>}
                  </g>
                );
              })}
            </svg>
          ...
          {selectedSkill && (
            <div style={{ width: 300, background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 12, padding: 20, flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                <div style={{ width: 12, height: 12, borderRadius: '50%', background: DOMAIN_COLORS[selectedSkill.domain] }} />
                <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--color-ink)', margin: 0 }}>{selectedSkill.name}</h3>
              </div>
              <div style={{ fontSize: '0.8rem', color: 'var(--color-ink-3)', marginBottom: 12 }}>
              ...
              {prereqChain.length > 0 && (
                <div style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--color-ink-3)', textTransform: 'uppercase', marginBottom: 4 }}>Prerequisites</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>{prereqChain.map(p => <span key={p} style={{ fontSize: '0.7rem', padding: '2px 6px', background: 'var(--color-paper-2)', borderRadius: 4, border: '1px solid var(--color-border)' }}>{p}</span>)}</div>
                </div>
              )}
              {downstream.length > 0 && (
                <div style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--color-ink-3)', textTransform: 'uppercase', marginBottom: 4 }}>Unlocks</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>{downstream.map(d => <span key={d} style={{ fontSize: '0.7rem', padding: '2px 6px', background: 'var(--color-accent-subtle)', borderRadius: 4, border: '1px solid var(--color-accent)', color: 'var(--color-accent)' }}>{d}</span>)}</div>
                </div>
              )}
              <div style={{ fontSize: '0.75rem', color: 'var(--color-ink-2)' }}>
                Job Demand: {Math.round((SKILL_DEMAND[selectedSkill.id] || 0.5) * 100)}%
              </div>
              {selectedSkill.resources?.length > 0 && (
                <div style={{ marginTop: 10 }}>
                  <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--color-ink-3)', textTransform: 'uppercase', marginBottom: 4 }}>Resources</div>
                  {selectedSkill.resources.slice(0, 2).map((r, i) => (
                    <div key={i} style={{ fontSize: '0.75rem', color: 'var(--color-ink-2)', marginBottom: 2 }}>• {r.title} ({r.platform})</div>
                  ))}
                </div>
              )}
            </div>
