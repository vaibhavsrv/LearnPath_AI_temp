    ...
    <div className="page-wrapper">
      ...
      <main className="container" style={{ paddingTop: 64, paddingBottom: 40 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          {domains.map(d => (
            <button key={d} className={`btn btn-sm`} style={{ background: filter === d ? DOMAIN_COLORS[d] : 'var(--bg-card)', color: filter === d ? '#fff' : 'var(--text)', border: `1px solid ${DOMAIN_COLORS[d]}` }} onClick={() => setFilter(d)}>
              {DOMAIN_NAMES[d] || d}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 16 }}>
          <div style={{ flex: 1, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', position: 'relative', height: 520, cursor: dragging ? 'grabbing' : 'grab' }} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onWheel={handleWheel}>
            ...
            <svg ref={svgRef} width="100%" height="100%" viewBox={`0 0 1200 800`} style={{ transform: `scale(${zoom}) translate(${pan.x / zoom}px, ${pan.y / zoom}px)` }}>
              <defs>
                <marker id="arrow" viewBox="0 0 10 6" refX="10" refY="3" markerWidth="8" markerHeight="6" orient="auto"><path d="M0,0 L10,3 L0,6" fill="var(--text-muted)" fillOpacity="0.3" /></marker>
              </defs>
              {skills.map(s => {
                ...
                return s.prerequisites.map(pId => {
                  ...
                  return <line key={`${s.id}-${pId}`} x1={to.x} y1={to.y} x2={from.x} y2={from.y} stroke="var(--text-muted)" strokeOpacity={visible ? 0.3 : 0.08} strokeWidth={1} markerEnd="url(#arrow)" />;
                });
              })}
              {skills.map(s => {
          ...
          {selectedSkill && (
            <div style={{ width: 300, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: 20, flexShrink: 0 }}>
              ...
              {prereqChain.length > 0 && (
                <div style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 4 }}>Prerequisites</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>{prereqChain.map(p => <span key={p} style={{ fontSize: '0.7rem', padding: '2px 6px', background: 'var(--bg-secondary)', borderRadius: 4, border: '1px solid var(--border)' }}>{p}</span>)}</div>
                </div>
              )}
              {downstream.length > 0 && (
                <div style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 4 }}>Unlocks</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>{downstream.map(d => <span key={d} style={{ fontSize: '0.7rem', padding: '2px 6px', background: 'var(--primary-subtle)', borderRadius: 4, border: '1px solid var(--primary)', color: 'var(--primary)' }}>{d}</span>)}</div>
                </div>
              )}
              ...
              {selectedSkill.resources?.length > 0 && (
                <div style={{ marginTop: 10 }}>
                  <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 4 }}>Resources</div>
                  {selectedSkill.resources.slice(0, 2).map((r, i) => (
                    <div key={i} style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 2 }}>• {r.title} ({r.platform})</div>
                  ))}
                </div>
              )}
            </div>
