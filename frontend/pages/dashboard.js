...
function FeedbackButtons({ skillId, onFeedback }) {
  ...
  return (
    <>
      {toast && <div style={{ position: 'fixed', bottom: 24, right: 24, background: 'var(--accent)', color: '#fff', padding: '12px 20px', borderRadius: 10, fontSize: '0.85rem', fontWeight: 600, zIndex: 9999, boxShadow: 'var(--shadow-lg)', animation: 'fadeIn 0.2s ease' }}>{toast}</div>}
...
function ScoringBreakdown({ recs }) {
  ...
  return (
    <div style={{ padding: 20, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, marginBottom: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div>
          <h3 style={{ fontSize: '0.95rem', fontWeight: 700 }}>Scoring Breakdown</h3>
          <p style={{ fontSize: '0.75rem', color: 'var(--text-3)' }}>How each recommendation is scored</p>
        </div>
        <Link href="/algorithm" style={{ fontSize: '0.75rem', color: 'var(--accent)', fontWeight: 600 }}>See Algorithm →</Link>
      </div>

      {recs.slice(0, 3).map((rec, ri) => (
        <div key={ri} style={{ marginBottom: ri < 2 ? 14 : 0, padding: 12, background: 'var(--bg-3)', borderRadius: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 600 }}>{rec.course.title}</span>
            <span style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--accent)' }}>{Math.round(rec.score * 100)}%</span>
          </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8, justifyContent: 'center' }}>
        {factors.map(f => (
          <div key={f.key} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.65rem', color: 'var(--text-3)' }}>
...
function ProjectsTracker({ profile, path }) {
  ...
  return (
    <div style={{ padding: 20, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, marginTop: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div>
          <h3 style={{ fontSize: '0.95rem', fontWeight: 700 }}>Projects & Assessments</h3>
          <p style={{ fontSize: '0.75rem', color: 'var(--text-3)' }}>{completedProjects}/{allSkills.length} projects completed</p>
        </div>
        <div style={{ display: 'flex', gap: 3, alignItems: 'center' }}>
          <div style={{ width: 80, height: 5, background: 'var(--bg-4)', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${allSkills.length ? (completedProjects / allSkills.length) * 100 : 0}%`, background: 'var(--green)', borderRadius: 3 }} />
          </div>
          <span style={{ fontSize: '0.7rem', fontWeight: 600, color: 'var(--green)' }}>{allSkills.length ? Math.round((completedProjects / allSkills.length) * 100) : 0}%</span>
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 8 }}>
        {allSkills.slice(0, 12).map((s, i) => (
          <div key={i} style={{ padding: '10px 12px', background: s.completed ? 'rgba(5,150,105,0.04)' : 'var(--bg-3)', borderRadius: 8, border: '1px solid ' + (s.completed ? 'rgba(5,150,105,0.15)' : 'var(--border)'), opacity: s.completed ? 0.7 : 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
              <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text)' }}>{s.project.title}</span>
              {s.completed && <span style={{ fontSize: '0.65rem', color: 'var(--green)', fontWeight: 600 }}>✓</span>}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.65rem', color: 'var(--text-3)' }}>
              <span style={{ padding: '1px 4px', borderRadius: 3, background: 'var(--accent-dim)', color: 'var(--accent)', fontWeight: 700, textTransform: 'uppercase' }}>Project</span>
              <span style={{ color: DOMAIN_COLORS[s.domain] || 'var(--text-3)' }}>{DOMAIN_NAMES[s.domain] || s.domain}</span>
...
function ProgressChart({ path, profile }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 24, padding: 20, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12 }}>
      <div style={{ position: 'relative', width: 100, height: 100, flexShrink: 0 }}>
        <svg width="100" height="100" viewBox="0 0 100 100">
          <circle cx="50" cy="50" r="42" fill="none" stroke="var(--bg-4)" strokeWidth="6" />
          <circle cx="50" cy="50" r="42" fill="none" stroke="var(--accent)" strokeWidth="6" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" transform="rotate(-90 50 50)" style={{ transition: 'stroke-dashoffset 0.6s ease' }} />
        </svg>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' }}>
          <div style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--accent)' }}>{pct}%</div>
        </div>
      </div>
      <div style={{ flex: 1 }}>
          ...
          {phases.map((p, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-3)', width: 80, flexShrink: 0 }}>Phase {p.phase}</span>
                <div style={{ flex: 1, height: 6, background: 'var(--bg-4)', borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${phasePct}%`, background: 'var(--accent)', borderRadius: 3, transition: 'width 0.5s ease' }} />
                </div>
                <span style={{ fontSize: '0.7rem', color: 'var(--text-3)', width: 30, textAlign: 'right' }}>{phaseCompleted}/{p.courses.length}</span>
              </div>
            );
          })}
        </div>
...
function NextActionCard({ profile, recs }) {
  ...
  const top = recs[0];
  return (
    <div style={{ padding: 20, background: 'var(--accent-dim)', border: '1px solid var(--accent)', borderRadius: 12, marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        <span style={{ fontSize: '1.2rem' }}>🎯</span>
        <h3 style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--accent)' }}>Next Recommended Action</h3>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <p style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--text)', marginBottom: 4 }}>{top.course.title}</p>
          <p style={{ fontSize: '0.8rem', color: 'var(--text-2)' }}>{top.course.provider} · {top.course.duration_hours}h · {top.course.level}</p>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--accent)' }}>{Math.round(top.score * 100)}%</div>
          <div style={{ fontSize: '0.7rem', color: 'var(--text-3)' }}>match</div>
        </div>
      </div>
      <p style={{ fontSize: '0.8rem', color: 'var(--text-2)', marginTop: 8, lineHeight: 1.5 }}>{top.explanation}</p>
      ...
      <main className="container" style={{ paddingTop: 64, paddingBottom: 40 }}>
        ...
        <div className="dashboard-grid" style={{ marginTop: 24 }}>
          <section className="dashboard-section">
            ...
            {recs.length === 0 && <p style={{ color: 'var(--text-3)', fontSize: '0.85rem' }}>Complete your profile to get recommendations.</p>}
            {recs.map((r, i) => (
