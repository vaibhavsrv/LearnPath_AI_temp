    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 6 }}>Input Queue</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {queue.length === 0 && !running && sorted.length === 0 && <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Click "Run" to start</span>}
            {queue.map(id => <span key={id} style={{ padding: '4px 8px', background: 'var(--primary-subtle)', border: '1px solid var(--primary)', borderRadius: 6, fontSize: '0.75rem', fontWeight: 600, color: 'var(--primary)' }}>{skillMap[id]?.name || id}</span>)}
          </div>
        </div>
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 6 }}>Sorted Output (Step {step})</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {sorted.map((id, i) => (
              <span key={id} style={{ padding: '4px 8px', background: i === sorted.length - 1 ? '#22c55e' : 'var(--bg-tertiary)', borderRadius: 6, fontSize: '0.75rem', fontWeight: 600, color: i === sorted.length - 1 ? '#fff' : 'var(--text)' }}>
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10, marginBottom: 16 }}>
        {[
          ...
          <div key={f.label} style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: 10, border: '1px solid var(--border)' }}>
            <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 4 }}>{f.label}</div>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: 16 }}>
        ...
        <div style={{ height: 20, background: 'var(--bg-tertiary)', borderRadius: 10, overflow: 'hidden', display: 'flex' }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Target: ML Engineer (demo profile)</span>
        <span style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--primary)' }}>{pct}% ready</span>
      </div>
      ...
      <div style={{ marginTop: 12, padding: 10, background: 'var(--bg-secondary)', borderRadius: 8, border: '1px solid var(--border)', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
    ...
    <div className="page-wrapper">
      ...
      <main className="container" style={{ paddingTop: 64, paddingBottom: 40 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          <section style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: 24 }}>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>1. Topological Sort (DAG Traversal)</h2>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: 16 }}>Kahn's algorithm ensures prerequisites are always learned before dependent skills. Watch nodes with zero in-degree get processed first.</p>
            <TopoDemo />
          </section>

          <section style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: 24 }}>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>2. 5-Factor Hybrid Scoring</h2>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: 16 }}>Each skill is scored on 5 weighted factors. Adjust the sliders to see how the final score changes.</p>
            <ScoringDemo />
          </section>

          <section style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: 24 }}>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>3. Skill Gap Analysis (Set Difference)</h2>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: 16 }}>Compares your current skills against career path requirements using set operations.</p>
            <GapDemo />
          </section>
        </div>
      </main>
